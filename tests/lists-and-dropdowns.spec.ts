...
test('Validate pet type update', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Pets and Visits' })).toBeVisible()
    await page.locator('dl').filter({ hasText: 'Rosy' }).getByRole('button', { name: 'Edit Pet' }).click()
    ...
    await expect(page.locator('dl').filter({ hasText: 'Rosy' })).toContainText('bird')
    ...
    await expect(page.locator('#type1')).toHaveValue('dog')
    await page.getByRole('button', { name: 'Update Pet' }).click()
    await expect(page.locator('dl').filter({ hasText: 'Rosy' })).toContainText('dog')
